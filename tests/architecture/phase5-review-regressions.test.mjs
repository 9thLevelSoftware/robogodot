...
test("Phase 5 runbook states runtime, bridge, DAP, failure, cleanup, and live verification boundaries", async () => {
  ...
  for (const pattern of [
    ...
    /exact child/i,
    /bridge[^\n]*before[^\n]*process/i,
    /Page-level `truncated: true` means more retained records remain/i,
    /record\.truncated: true` means that record's line was shortened/i,
    /Natural exit[^\n]*running:false[^\n]*final drain/i,
    /failed close[^\n]*retryable[^\n]*blocks a new launch/i,
    /Godot 4\.6 does not expose a portable link count/i,
    /dummy renderer/i,
    /GODOT_DAP_PORT[^\n]*6006/,
