...
func _run() -> void:
	...
	_check(router.register_command("test.huge", func(_params): return {"ok": true, "result": {"text": "é".repeat(140000)}}), "huge registration must succeed")
	var escaped_id := String.chr(1).repeat(Router.MAX_REQUEST_ID_BYTES)
	var bounded_huge: Dictionary = router.dispatch({"jsonrpc":"2.0", "id":escaped_id, "method":"test.huge", "params":{}})
	_check(bounded_huge.has("error") and bounded_huge.id == escaped_id and JSON.stringify(bounded_huge).to_utf8_buffer().size() <= Router.MAX_RESPONSE_BYTES, "router replaces oversized success with bounded same-id error including escaped id")
