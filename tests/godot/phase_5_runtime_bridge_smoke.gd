...
func _run() -> void:
	...
	bridge.cleanup(); await process_frame
	for name in DirAccess.get_files_at(session_root): _assert(not name.begins_with("req-") and not name.begins_with("resp-") and not name.begins_with(".req-") and not name.begins_with(".resp-"), "bridge artifact cleanup")
	InputMap.erase_action("runtime_smoke_action"); bridge.queue_free(); runtime_root.queue_free()
...
func _request(id: int, method: String, params: Dictionary) -> Dictionary:
	...
	var path := session_root.path_join("req-%d.json" % id); var file := FileAccess.open(path, FileAccess.WRITE); file.store_string(JSON.stringify(request)); file.close()
