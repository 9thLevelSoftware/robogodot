...
liveDescribe("Phase 5 public MCP runtime and attach-only DAP acceptance", () => {
  ...
  test("debug run attaches, breaks, inspects local 42, steps, continues, and stops", async () => {
    ...
    try {
      ...
      if (!stack) throw new Error(`Verified breakpoint never produced a stopped DAP state; last stack error: ${lastStackError instanceof Error ? lastStackError.message : String(lastStackError)}`);
      const fixtureFrame = stack.frames.find((candidate: any) => candidate.source?.path?.endsWith("phase5/runtime_fixture.gd")); expect(fixtureFrame).toBeDefined(); const frame = fixtureFrame.ref, thread = stack.threads[0].ref;
      const scopes = await waitCall<any>(h.client, "godot_debug_inspect", { sessionId, frame, start: 0 }); let found = false; for (const scope of scopes.scopes) { const values = await waitCall<any>(h.client, "godot_debug_inspect", { sessionId, frame, variables: scope.ref, start: 0 }); if (values.variables.some((value: any) => value.name === "phase5_value" && value.value === "42")) found = true; } expect(found).toBe(true);
