...
describe("public runtime process tools", () => {
  it("registers exactly three tools with reviewed annotations", async () => { const h = await harness(); try { const tools = (await h.client.listTools()).tools.filter(t => ["godot_run_project", "godot_stop_project", "godot_run_output"].includes(t.name)); expect(tools.map(t => t.name)).toEqual(["godot_run_project", "godot_stop_project", "godot_run_output"]); expect(tools.map(t => t.annotations)).toEqual([
    ...
    ]); const error = await h.client.callTool({ name: "godot_run_project", arguments: {} }); expect(error.isError).toBe(true); expect(error.structuredContent).toBeUndefined(); expect(JSON.parse((error.content as any)[0].text)).toEqual({ code: "not_connected", message: "The runtime process service is not configured.", hint: "Configure Godot and the runtime session coordinator before using process tools." }); } finally { await h.close(); } });
  ...
  it("publishes exact success-or-standard-error schemas and copies only guarded own data", async () => {
    ...
    const runtime = { launch: vi.fn().mockResolvedValue(throwing), stop: vi.fn(), output: vi.fn() }; const bad = await harness(runtime); try { await bad.client.listTools(); const result = await bad.client.callTool({ name: "godot_run_project", arguments: {} }); expect(result.isError).toBe(true); expect(result.structuredContent).toBeUndefined(); expect(JSON.parse((result.content as any)[0].text)).toEqual({ code: "godot_error", message: "Runtime service returned an invalid response.", hint: "Check that the runtime coordinator and MCP server versions are compatible." }); } finally { await bad.close(); }
