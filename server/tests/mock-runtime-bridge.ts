...
interface Options { sessionId: string; token: string; hold?: boolean; sessionRoot?: string }
export class MockRuntimeBridge {
  port = 0; requests: Record<string, any>[] = []; ids: number[] = []; beforeResponse?: (request: Record<string, any>) => Record<string, any>[];
  ...
  private async listen() { this.server = createServer(socket => { this.sockets.add(socket); const d = new FrameDecoder(); let authenticated = false; socket.on("data", chunk => { for (const raw of d.push(chunk)) { const v = raw as Record<string, any>; if (!authenticated) { if (v.type === "hello" && v.version === 1 && v.sessionId === this.options.sessionId && v.token === this.options.token) { authenticated = true; socket.write(encodeFrame({ type: "hello_ack", version: 1, sessionId: this.options.sessionId })); } else socket.destroy(); continue; } void this.respond(v, x => socket.write(encodeFrame(x))); } }); }); await new Promise<void>(r => this.server!.listen(0, "127.0.0.1", r)); this.port = (this.server.address() as any).port; }
