...
describe("DapClient", () => {
  ...
  it("gates advertised capabilities", async () => { const mock = await server(); const client = new DapClient(); const attaching = client.attach({ host: "127.0.0.1", port: mock.port, runtimeSessionId: SESSION, process: { pid: 2 } }); await respondHandshake(mock, {}); await expect(attaching).rejects.toMatchObject({ code: "feature_disabled" }); await expect(client.terminate()).rejects.toMatchObject({ code: "feature_disabled" }); });
  it("binds threads, frames, scopes, and variables to the stopped generation with bounded pagination", async () => {
    ...
    const stacked = client.stack(); await expect.poll(() => mock.messages.at(-1)?.command).toBe("threads"); mock.respond(mock.messages.at(-1), { threads: Array.from({ length: 70 }, (_, i) => ({ id: i + 1, name: `thread ${i}` })) }); await expect.poll(() => mock.messages.at(-1)?.command).toBe("stackTrace"); expect(mock.messages.at(-1).arguments.levels).toBe(256); mock.respond(mock.messages.at(-1), { stackFrames: [{ id: 9, name: "main", line: 4, column: 1, source: { path: "C:/game/main.gd" } }], totalFrames: 1 });
    const stack = await stacked; expect(stack.threads).toHaveLength(64); expect(stack.threads[0].ref).toEqual({ runtimeSessionId: SESSION, stoppedGeneration: 1, id: 1 }); expect(stack.frames[0].ref).toEqual({ runtimeSessionId: SESSION, stoppedGeneration: 1, id: 9 });
