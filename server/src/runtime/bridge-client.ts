...
export class RuntimeBridgeClient {
  ...
  private async connectSocket(secret: SecretConfig): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      let serverNonce = ""; let confirmed = false;
      const data = (chunk: Buffer) => { try { for (const raw of this.decoder.push(Buffer.from(chunk))) { const value = raw as Record<string, unknown>; if (!confirmed) { const nonce = value.serverNonce; const serverProof = value.serverProof; if (value.type !== "hello_ack" || value.version !== secret.protocolVersion || value.sessionId !== secret.sessionId || value.clientNonce !== clientNonce || typeof nonce !== "string" || !/^[a-f0-9]{64}$/.test(nonce) || typeof serverProof !== "string" || !fixedProof(serverProof, proof(secret.token, "robogodot-server-v1", secret.sessionId, secret.protocolVersion, clientNonce, nonce))) throw new Error("Invalid runtime bridge mutual proof."); serverNonce = nonce; confirmed = true; socket.write(encodeFrame({ type: "hello_confirm", version: secret.protocolVersion, sessionId: secret.sessionId, clientNonce, serverNonce, confirmation: proof(secret.token, "robogodot-confirm-v1", secret.sessionId, secret.protocolVersion, clientNonce, serverNonce) }), error => { if (error) finish(new Error("Runtime bridge confirmation failed.")); }); continue; } const readyProof = value.readyProof; if (value.type !== "hello_ready" || value.version !== secret.protocolVersion || value.sessionId !== secret.sessionId || value.clientNonce !== clientNonce || value.serverNonce !== serverNonce || typeof readyProof !== "string" || !fixedProof(readyProof, proof(secret.token, "robogodot-ready-v1", secret.sessionId, secret.protocolVersion, clientNonce, serverNonce))) throw new Error("Invalid runtime bridge readiness proof."); finish(); } } catch (e) { finish(e as Error); } };
