...
export function registerSignalTools(server: McpServer, bridge: CoreBridge, lane: MutationLane): void {
  ...
  registerTool(server, { name: "godot_signal_connect", description: "Undoable signal connection mutation.", inputSchema: z.object({ ...base, flags: z.number().int().safe().min(0).max(15).default(0) }).strict(), outputSchema: mutationOutput, annotations: mutate, handler: (i: any) => lane.run(["signals", `node:${i.source}`, `node:${i.callable.target}`], () => callCurated(bridge, "edit.signal_connect", i, mutationOutput)) });
  registerTool(server, { name: "godot_signal_disconnect", description: "Undoable signal disconnection mutation.", inputSchema: z.object(base).strict(), outputSchema: mutationOutput, annotations: mutate, handler: (i: any) => lane.run(["signals", `node:${i.source}`, `node:${i.callable.target}`], () => callCurated(bridge, "edit.signal_disconnect", i, mutationOutput)) });
}
