...
export function registerNodeTools(server: McpServer, bridge: CoreBridge): void {
  const mutation = (name: string, method: string, schema: z.ZodObject, output: z.ZodObject) => registerTool(server, {
    ...
    handler: async (input: any) => callCurated(bridge, method, input, output),
  });
  mutation("godot_node_add", "edit.node_add", z.object({ parent: nodePath, type: className, name: nodeName, properties: z.record(propertyName, variantLiteralSchema).default({}) }).strict(), pathResponse);
  mutation("godot_node_delete", "edit.node_delete", z.object({ path: nodePath }).strict(), pathResponse);
  mutation("godot_node_reparent", "edit.node_reparent", z.object({ path: nodePath, parent: nodePath, index: z.number().int().nonnegative().optional() }).strict(), pathResponse);
  mutation("godot_node_rename", "edit.node_rename", z.object({ path: nodePath, name: nodeName }).strict(), pathResponse);
  mutation("godot_node_duplicate", "edit.node_duplicate", z.object({ path: nodePath, parent: nodePath.optional(), name: nodeName.optional(), flags: z.number().int().nonnegative().default(15) }).strict(), pathResponse);
  ...
  mutation("godot_node_set_property", "edit.node_set_property", z.object({ path: nodePath, property: propertyName, value: variantLiteralSchema }).strict(), setResponse);
  registerTool(server, {
  ...
  mutation("godot_scene_instance", "edit.node_instance", z.object({ parent: nodePath, scenePath: projectPath, name: nodeName.optional() }).strict(), z.object({ path: nodePath, type: className, scenePath: projectPath }).strict());
}
