...
const projectPath = z.string().min(7).max(1024).refine(path => {
...
const currentResponse = z.object({ path: z.union([projectPath, z.literal("")]), unsaved: z.boolean() }).strict();
const treeNode: z.ZodType<any> = z.lazy(() => z.object({ name: z.string(), class: z.string(), path: nodePath, children: z.array(treeNode) }).strict());
