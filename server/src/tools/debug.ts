...
const launchOutput = z.object({ sessionId, mode: z.literal("debug"), state: z.literal("debug_ready"), pid: z.number().int().positive(), startedAt: z.number(), bridgeTransport: z.enum(["socket", "file"]).optional() }).strict();
...
function normalizeLaunch(value: unknown): Record<string, unknown> { const result = normalize(value); const output: Record<string, unknown> = { sessionId: typeof result.sessionId === "string" ? result.sessionId : result.id, mode: result.mode, state: result.state, pid: result.pid, startedAt: result.startedAt }; if (result.bridgeTransport !== undefined) output.bridgeTransport = result.bridgeTransport; return output; }
