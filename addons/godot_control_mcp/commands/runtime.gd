...
static func prepare(params: Dictionary) -> Dictionary:
	...
	if not (protocol_version is int or protocol_version is float) or not is_finite(float(protocol_version)) or floorf(float(protocol_version)) != float(protocol_version) or int(protocol_version) != Manifest.PROTOCOL_VERSION: return _failure("protocolVersion is unsupported.")
	if not (preferred_port is int or preferred_port is float) or not is_finite(float(preferred_port)) or floorf(float(preferred_port)) != float(preferred_port) or preferred_port < 1 or preferred_port > 65535: return _failure("preferredPort must be an integer from 1 to 65535.")
