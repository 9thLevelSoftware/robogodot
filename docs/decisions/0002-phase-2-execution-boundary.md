...
- Editor-script execution is blocked in `read_only` and `confirm_destructive`. In `full`, every execution requires the explicit per-request capability flag `allowDangerous:true`, regardless of source-text heuristics. Deny-list matches are diagnostics only and never grant or deny authorization.
